...
func TestPrintPrimeContent_NilReport(t *testing.T) {
	...
	if !strings.Contains(output, "# Tpg CLI Context") {
...
func TestPrintPrimeContent_WithReport(t *testing.T) {
	...
	if !strings.Contains(output, "Run 'tpg ready [-p project]'") {
...
func TestPrintPrimeContent_EmptyReport(t *testing.T) {
	...
	// Should prompt for ready command
	if !strings.Contains(output, "Run 'tpg ready [-p project]'") {
		t.Error("should prompt to run tpg ready")
	}
}
...
func TestPrintPrimeContent_ContextRetrieval(t *testing.T) {
	...
	// Should indicate that tpg done prompts for reflection
	if !strings.Contains(output, "tpg done") {
		t.Error("missing tpg done command")
	}
	if !strings.Contains(output, "will prompt for reflection") {
		t.Error("should indicate tpg done prompts for reflection")
	}
}
