...
Always use `tpg show`, `tpg dep list`, and `tpg ready` to understand task state and relationships.
...
tpg dep list <blocked-task> -t blocks
...
tpg add "Resolve: [blocker description]" -t task -p 1

# Link it
tpg dep add <blocked-task> <blocker-task>
...
When launching tpg-agent, provide:
1. **Task ID** (required) - The tpg task to work on
2. **Template context** (if applicable) - Pattern name and variables
3. **Brief context hint** (optional) - Only if task description is thin
...
```
@tpg-agent Work on ORDERS-1.2: Implement Order CRUD.
Template: crud-module (entity=Order, table=orders)
Custom focus: Order status validation, payment integration hooks.
```

**Minimal (task has full context):**
```
@tpg-agent AUTH-1.2
```

The agent will gather all context from tpg. Only add hints if you know the task description is incomplete.
...
tpg add "Replace payment API mock with real integration" \
  -t task -p 2 \
  --blocks TASK-142 \
  -d "In src/services/payment.ts we added a mock. Replace with real Stripe integration."
```
...
tpg label add AUTH-1.2 needs-review
tpg label add AUTH-1.2 needs-tests

# Remove labels when requirements are met
tpg label rm AUTH-1.2 needs-review
```
...
**Task IDs are meaningless:** Never infer order, relationships, or priority from ID patterns. Always use `tpg ready`, `tpg show`, and `tpg dep list` to understand state.
