...
func TestFindTemplatesDir(t *testing.T) {
	t.Run("finds templates dir in current directory", func(t *testing.T) {
		tmpDir := t.TempDir()
		...
		result, err := FindTemplatesDir()
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		// Use EvalSymlinks to resolve /var -> /private/var on macOS
		expectedResolved, _ := filepath.EvalSymlinks(templatesDir)
	...
	t.Run("finds templates dir in ancestor directory", func(t *testing.T) {
		tmpDir := t.TempDir()
		templatesDir := filepath.Join(tmpDir, ".tgz", "templates")
		if err := os.MkdirAll(templatesDir, 0755); err != nil {
			t.Fatalf("failed to create templates dir: %v", err)
	...
	t.Run("returns error when templates dir not found", func(t *testing.T) {
		tmpDir := t.TempDir()
		...
		if !strings.Contains(err.Error(), "no templates directory found") {
			t.Errorf("unexpected error message: %v", err)
		}
	})

	t.Run("returns error when templates path is file not directory", func(t *testing.T) {
		tmpDir := t.TempDir()
		tgzDir := filepath.Join(tmpDir, ".tgz")
		if err := os.MkdirAll(tgzDir, 0755); err != nil {
			t.Fatalf("failed to create .tgz dir: %v", err)
		}

		// Create templates as a file instead of directory
		templatesFile := filepath.Join(tgzDir, "templates")
		if err := os.WriteFile(templatesFile, []byte("not a directory"), 0644); err != nil {
			t.Fatalf("failed to create templates file: %v", err)
		}
